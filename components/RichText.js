	...
	return Object.keys(parts).map((part, i, partsAr) => {
		...
		return segment_map[i].type === "hyperlink" && button ? (
			<React.Fragment>
				{isWeb ? (
					<Link key={i} href={url}>
						<Button color={Theme.green} title={parts[part] || ""} />
					</Link>
				) : (
					<View>
						<Button
			...
			<Text
				...
				style={[body_style, segment_map[i].type === "strong" ? { fontWeight: "bold", color: Theme.green } : { color: Theme.green }]}
			>
				{parts[part] || ""}
			</Text>
		) : (
...
export function RichText(props) {
	...
	function header(part, key, ar) {
		...
		return bullet_header.indexOf(part.type) > -1 ? (
			<View key={key} style={{ position: "relative", marginBottom: 10 }}>
				<Text accessibilityRole="header" aria-level={aria_level} style={[text_style, { marginTop: 40 }]}>
					- {part.text}
				</Text>
			...
			<Text key={key} accessibilityRole="header" aria-level={aria_level} style={[text_style, { marginTop: 40 }]}>
				{part.text}
			</Text>
		);
	}
	...
	function part(part, key, ar) {
		if (part.type.indexOf("heading") > -1 && part.text) {
			...
			return (
				<Text key={key} style={[body_style, { marginTop: 10, marginBottom: 10 }]}>
