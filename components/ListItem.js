    ...
    return (
        <Link href={'/biz/' + listing.uid}>
            <View style={{borderBottomWidth:2, borderColor: Theme.green, padding: 20, flexDirection: 'row'}}>
                <View style={{flex: 1}}>
                    <ResponsiveImage style={{width: 800, resizeMode: 'contain', aspectRatio: 1}} source={{uri: listing.primary_image.url + '&w=800'}} />
                </View>
                <View style={{flex: 3, paddingLeft: 20}}>
                    <Text style={styles.text_header4}>{listing.name}</Text>
                    <Text style={[styles.text_body2,{paddingTop: 20, paddingBottom:20}]}>{listing.description}</Text>
                    ...
                    {!!listing.address && <Text style={[styles.text_body2,{paddingTop: 20, paddingBottom:20}]}>{listing.address}</Text>}
                </View>
            </View>
        </Link>
    )
