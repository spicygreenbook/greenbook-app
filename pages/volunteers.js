...
export async function getStaticProps(context) {
    let content = await getContent({type: 'content', uid: 'volunteers', ref_id: context.preview || ''});
    let volunteers = await getData({type: 'volunteers'})

    return {
        props: {content: content && content.content || {}, volunteers: volunteers, url: '/volunteers'}
    };
}
